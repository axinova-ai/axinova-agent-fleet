        ...
        case "$choice" in
            ...
            3)
                if [[ -f "$qr_txt_file" ]]; then
                    ...
                    cat "$qr_txt_file"
                    echo ""
                    log_info "Scan with WireGuard mobile app"
                    read -p "Press Enter to continue..."
                else
...
show_distribution_instructions() {
    ...
    case "$device_type" in
        macos)
            ...
            echo ""
            echo "5. Enable at boot (optional):"
        ...
        windows)
            ...
            echo ""
            echo "2. Install WireGuard for Windows from:"
            echo "   https://www.wireguard.com/install/"
            echo ""
            echo "3. Import configuration:"
            echo "   - Open WireGuard GUI application"
        ...
        ios|android)
            ...
            if [[ "$device_type" == "ios" ]]; then
                echo "   - iOS: https://apps.apple.com/app/wireguard/id1441195209"
            else
                echo "   - Android: https://play.google.com/store/apps/details?id=com.wireguard.android"
            fi
            echo ""
