...
- Tasks in Needs Founder that could be picked up manually via `$pickup-task` or re-routed
- Tasks in In Review with no PR link (may need manual check)

## Notes

- Use comments to identify claiming agents, PR URLs, and escalation causes.
- The launcher currently escalates failed Codex code-task runs directly to Needs Founder instead of falling back to Kimi.
